...
class TemporalConv(nn.Module):
    ...
    def __init__(self, in_chans=1, out_chans=8):
        super().__init__()
        self.conv1 = nn.Conv1d(in_chans, out_chans, kernel_size=15, stride=1, padding=7)               #15/25/45
        self.gelu1 = nn.GELU()
...
class Graph_Fusion(nn.Module):
    ...
    def forward(
        ...
        x_cnn = self.cnn(x)  # 经过TemporalConv处理
        ...
        #导入先验的邻接矩阵
        prior_adj_mat = torch_geometric.utils.to_dense_adj(
        ...
        #合并
        combined_adj = 0.5 * prior_adj_mat + 0.5 * adj_mat
        # prune graph
        combined_adj = prune_adj_mat(
            combined_adj,
            num_nodes,
        ...
        # regularization loss
        reg_losses = self.regularization_loss(x, adj=combined_adj)

        # back to sparse graph
        edge_index, edge_weight = torch_geometric.utils.dense_to_sparse(combined_adj)
